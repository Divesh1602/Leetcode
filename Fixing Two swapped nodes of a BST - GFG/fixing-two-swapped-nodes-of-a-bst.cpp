...
class Solution {
  public:
   //function to get the inorder array of the wrong BST
    void inorder_arr(Node* root,vector<int>&val){
        if(root==NULL){
            return;
        }
        inorder_arr(root->left,val);
        val.push_back(root->data);
        inorder_arr(root->right,val);
    }
    
    //correct the BST
    void correct_BST(Node* root,vector<int>&val,int &p){
        if(root==NULL){
            return;
        }
        //call the left side
        correct_BST(root->left,val,p);
        //set the value
        root->data=val[p];
        //index increase
        p++;
        //call the right side
        correct_BST(root->right,val,p);
        
    }
    struct Node *correctBST(struct Node *root) {
        vector<int>curr_order;
        //taking the BST with wrong nodes
        inorder_arr(root,curr_order);
        //sort the nides with the correct order to get the correct BST
        sort(curr_order.begin(),curr_order.end());
        //for array inde
        int p=0;
        correct_BST(root,curr_order,p);
